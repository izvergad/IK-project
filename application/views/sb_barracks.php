...
?>
    ...
    <div class="currentUnit <?=$this->Data_Model->army_class_by_type($this->Town_Model->army_line[0]['type'])?>">
        <div class="amount"><span class="textLabel"><?=$this->Data_Model->army_name_by_type($this->Town_Model->army_line[0]['type'])?></span></div>
...
<script type="text/javascript">
	...
	var tmppbar = getProgressBar({
		startdate: <?=$this->Town_Model->army_start?>,
		enddate: <?=$end_date?>,
		currentdate: <?=time()?>,
		bar: "buildProgress"
		});
...
<?if ($count > 1){?>
<?
        $end_date = $this->Town_Model->army_start + $cost['time']*($count-1);
        $ostalos = $end_date - time();
        if ($ostalos < 0){ $ostalos = 0; }
        $ostalos_all = $ostalos_all + $ostalos;
?>
    <li class="<?=$this->Data_Model->army_class_by_type($this->Town_Model->army_line[0]['type'])?>">
        <div class="amount"><?=$count-1?><span class="textLabel"> <?=$this->Data_Model->army_name_by_type($this->Town_Model->army_line[0]['type'])?></span></div> 
...
<?
        ...
        $ostalos_all = $ostalos_all + $ostalos;
?>
    <li class="<?=$this->Data_Model->army_class_by_type($this->Town_Model->army_line[$i]['type'])?>">
        <div class="amount"><?=$this->Town_Model->army_line[$i]['count']?><span class="textLabel"> <?=$this->Data_Model->army_name_by_type($this->Town_Model->army_line[$i]['type'])?></span></div>
        <div class="time" id="queueEntry1"><?=format_time($ostalos_all)?><span class="textLabel"> до завершения</span></div>
    </li>
<?}?>
    </ul>
