                       ...
                       if (($this->CI->Update_User->towns[$i]->build_start + $cost['time']) <= time())
                       {
                           if (($step == 0) or ($step > 0 and $wood >= 0 and $marble >= 0 and $wine >= 0 and $crystal >= 0 and $sulfur >= 0))
                           {
                                ...
                                if ($step > 0)
                                {
                                    ...
                                    $message = ($this->CI->Update_User->towns[$i]->$pos_text == 1) ? 'Строительство "<a href="'.$this->config->item('base_url').'game/city/'.$this->CI->Update_User->towns[$i]->id.'/'.$this->Data_Model->building_class_by_type($buildings[0]['type']).'/'.$buildings[0]['position'].'/">'.$this->Data_Model->building_name_by_type($buildings[0]['type']).'</a>" завершено!' : 'Уровень здания "<a href="'.$this->config->item('base_url').'game/city/'.$this->CI->Update_User->towns[$i]->id.'/'.$this->Data_Model->building_class_by_type($buildings[0]['type']).'/'.$buildings[0]['position'].'/">'.$this->Data_Model->building_name_by_type($buildings[0]['type']).'</a>" увеличен до '.$this->CI->Update_User->towns[$i]->$pos_text.'!';
